...
nnz(testpreds == temp.Labels)/numel(testpreds)
% Visualize the confusion matrix
cm = confusionchart(temp.Labels,testpreds,'RowSummary','row-normalized');
...
figure
for i = 1:4
    subplot(2,2,i);imshow(readimage(temp,idx(i)));title(string(temp.Labels(idx(i))) + ", " + num2str(100*max(scores(idx(i),:)),3) + "%");
end