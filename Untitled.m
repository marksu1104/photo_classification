p = genpath('C:\Users\marks\桌面\data\data');% 獲得資料夾data下所有子檔案的路徑，這些路徑存在字串p中，以';'分割
...
for i = 1:file_num
    ...
    if img_num > 0
        for j = 1:img_num
            ...
            width=size(image,2); %获取图像宽
            length1=size(image,1); %获取图像长
            if(width ~= 227 || length1 ~= 227)
                fprintf('%d %d %s\n',i,j,strcat(file_path,image_name))
                disp(width);disp(length1);
